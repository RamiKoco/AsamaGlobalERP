    ...
    public partial class GelirDagilimRaporu : BaseRapor
    {
        ...
        protected override void Tablo_CustomSummaryCalculate(object sender, CustomSummaryEventArgs e)
        {
            ...
            if (item.FieldName != "colTahsilatOrani") return;
                ...
                if (e.IsGroupSummary)
                {
                    var odemelerToplami = Convert.ToDecimal(Tablo.GetGroupSummaryValue(e.GroupRowHandle,
                        (GridGroupSummaryItem)Tablo.GroupSummary["NetOdeme"]));
                    var tahsilatlarToplami = Convert.ToDecimal(Tablo.GetGroupSummaryValue(e.GroupRowHandle,
                        (GridGroupSummaryItem)Tablo.GroupSummary["Tahsil"]));
