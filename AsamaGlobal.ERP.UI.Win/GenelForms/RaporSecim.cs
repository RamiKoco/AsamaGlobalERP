...
using AsamaGlobal.ERP.UI.Win.Reports.XtraReports.Fatura;
using AsamaGlobal.ERP.UI.Win.Reports.XtraReports.Makbuz;
using AsamaGlobal.ERP.UI.Win.Reports.XtraReports.Tahakkuk;
...
namespace AsamaGlobal.ERP.UI.Win.GenelForms
{
    public partial class RaporSecim : BaseListForm
    {
        ...
        protected override void DegiskenleriDoldur()
        {
            ...
            if (_raporBolumTuru == RaporBolumTuru.FaturaDonemRaporlari ||
                _raporBolumTuru == RaporBolumTuru.FaturaGenelRaporlar ||
                _raporBolumTuru == RaporBolumTuru.MakbuzRaporlari)
            {
                switch (_raporBolumTuru)
                {
                    case RaporBolumTuru.MakbuzRaporlari:
                    {
                        var showItems = new BarItem[]
                            { btnGenelMakbuz, btnTahsilatMakbuzu, btnTeslimatMakbuzu, btnGeriIadeMakbuzu };
        ...
        private void ReportDataSource(IReport rapor)
        {
            switch (rapor)
            {
                case OgrenciKartiRaporu rpr:
                    ...
                    rpr.Indirim_Bilgileri.DataSource =
                        _indirimBilgileri.GroupBy(x => new { x.IndirimAdi, x.IptalTarihi, x.IslemTarihi })
                            .Select(x => new
                            {
                                x.Key.IndirimAdi,
                                x.Key.IptalTarihi,
