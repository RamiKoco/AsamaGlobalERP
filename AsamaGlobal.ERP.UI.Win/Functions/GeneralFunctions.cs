...
using AsamaGlobal.ERP.UI.Win.Properties;
   ...
   public static class GeneralFunctions
    {
        ...
        public static byte[] ResimYukle()
        {
            ...
            {
                Title = "Resim Seç",
                Filter = "Resim Dosyaları (*.bmp, *.gif, *.jgp, *.png)|*.bmp; *.gif; *.jpg; *png|Bmp Dosyaları|*.bmp|Gif Dosyaları|*.gif|Jpg Dosyaları|*.jpg|Png Dosyaları|*.png",
                InitialDirectory = @"C:\",
        ...
        public static void CreateConnectionString(string initialCatalog, string server, SecureString kullaniciAdi, SecureString sifre, YetkilendirmeTuru yetkilendirmeTuru)
        {
            ...
            var configuration = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            configuration.ConnectionStrings.ConnectionStrings["ERPContext"].ConnectionString =
        ...
        public static (SecureString secureSifre, SecureString secureGizliKelime, string sifre, string gizliKelime) SifreUret()
        {
            string RandomDegerUret(int minValue, int count)
            {
                ...
                const string karakterTablosu = "0123456789ABCDEFGHIJKLMNOPQRSTUVXWYZabcdefghıjklmnopqrstuvxwyz";
                string sonuc = null;
        ...
        public static bool SifreMailiGonder(this string kullaniciAdi, string rol, string email,
            SecureString secureSifre, SecureString secureGizliKelime)
        {
            ...
            using (var bll = new MailParametreBll())
            {
                ...
                {
                    From = new MailAddress(entity.Email, "Asama Global ERP Programı"),
                    To = { email },
                    Subject = "Asama Global ERP Programı Kullanıcı Bilgileri",
                    IsBodyHtml = true,
                    Body = "Asama Global ERP Programına Giriş İçin Gereken Kullanıcı Adı, Şifre, Gizli Kelime Bilgileri Aşağıdadır.<br/>" + 
        ...
        public static bool YetkiKontrolu(this KartTuru kartTuru, YetkiTuru yetkiTuru)
        {
            ...
            using (var bll = new RolYetkileriBll()) yetkiler = AnaForm.DonemParametreleri.YetkiKontroluAnlikYapilacak ? bll.Single(x => x.RolId == AnaForm.KullaniciRolId && x.KartTuru == kartTuru).EntityConvert<RolYetkileri>() : AnaForm.RolYetkileri.FirstOrDefault(x => x.KartTuru == kartTuru);
